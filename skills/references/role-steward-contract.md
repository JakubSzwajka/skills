...
6. If a gap needs longer decision ping-pong, recommend the relevant steward session (`product-owner`, `architect`, or `designer`) and include a copy-pasteable `Fresh steward session prompt` the parent can give the user. For validation/release-confidence gaps, recommend a bounded `engineering/quality/test` or `engineering/quality/review` pass, or escalate to the CTO when the decision is portfolio-level technical discipline.
7. The prompt must ask the fresh steward session to interview/grill the user, accept links/docs/screens/examples, challenge terminology drift, cross-check code/docs, converge on shared decisions, save/update the canonical owned docs listed above, and return guidance for `prd-create` and default execution.
...
End with: decisions made, docs updated, remaining blockers, and guidance for prd-create/default execution.
~~~
...
Use `STATUS: OK` when the domain has enough documented guidance for the requested work, even if follow-up doc updates are recommended.
Use `STATUS: BLOCKED` when external access/tooling/context prevents useful analysis.
Use `STATUS: STOP` when the current plan should not proceed even with defaults.
Use `Proposed doc updates` and `Next` for follow-up knowledge work instead of creating separate status words.
