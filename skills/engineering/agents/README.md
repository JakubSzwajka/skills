...
Implementation and validation are handled by default Codex execution. Do not add generic `developer` or `qa` personas here; keep those duties in the execution contract and helper workflows.
...
Quality helper workflows live outside `agents/` under `../quality/`:
