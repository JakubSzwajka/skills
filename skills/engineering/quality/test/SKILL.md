...
name: test
description: >
  Quality helper for running tests and producing a focused report. Use when the user
  asks for a test run or when a workflow needs validation evidence. Auto-detects test
  framework (pytest, jest, go test, cargo test, etc.), filters noise, and returns
  actionable results.
user-invocable: true
...
- Return `Status: BLOCKED` with `Result: no test framework found` so the caller knows the situation.
...
Produce a focused report. If this is part of a handoff, include the same facts in the compact shape from `../../../references/handoff-packet.md`.
