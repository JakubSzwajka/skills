...
name: review
description: >
  Quality helper for pre-push code review. Use when the user asks for review or when
  a workflow needs independent validation of a changeset. Analyzes git changes for
  correctness, test coverage, types, naming, architecture patterns, backwards
...
The reviewer evaluates changes against existing product/design/architecture/development doctrine. If doctrine is missing or ambiguous, report the gap and escalate to the owning steward rather than creating new doctrine.
...
```
  ...
  task: |
    ...
    **Scope**: <N files changed, +X/-Y lines>
    ...
    Status rules:
    - OK — no BLOCKERs and no open WARNs
    - NEEDS_INPUT — review found a product/design/architecture decision the reviewer cannot own
    - BLOCKED — required diff, files, tests, or runtime context could not be inspected
    - STOP — correctness bug, data loss risk, security issue, or open WARN that should be fixed before push
    
    Do NOT declare OK when WARNs are listed.
...
The subagent's output IS the review. Present it directly to the user — do not rewrite or summarize it. If a compact handoff is useful, add only the shared `../../../references/handoff-packet.md` shape after the review, without changing the reviewer status.

## After Review

Based on the status, suggest one of:
- **OK** → "Want me to commit these changes?"
- **NEEDS_INPUT** → "This needs an owner decision before I change code. Want me to frame the decision?"
- **BLOCKED** → "I could not complete the review because of missing context/tooling. Want me to gather that first?"
- **STOP** → "These need fixing before push. Want me to start with the blockers?"
