...
One template for every persona. Substitute every `<<placeholder>>` and match the cadence + section order of the existing decision personas (architect, designer, product-owner).

Three places inside the template have an advisory/execution variant — pick the one that fits the role. Do not create a generic developer or QA persona; default Codex execution already owns ordinary implementation and validation.
