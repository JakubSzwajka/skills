...
- Read `~/.agents/ORCHESTRATION.md` at the start of any task that will change more than one file, and follow it.
- You orchestrate. You do not implement. Every handoff names the paths a child owns, and you do not open those paths afterwards.
- A read-only reviewer never applies its own findings, and neither do you. Launch a writer that owns the files, then a fresh verifier that did not write the code.
- Parallel fan-out is read-only by default. Concurrent writers require explicit operator approval and one isolated worktree per writer.
- A child that pauses for you is answered before anything else. A child that is running needs nothing.
