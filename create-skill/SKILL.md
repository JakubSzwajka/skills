...
Use Socratic questioning to understand the skill before building it. Ask 2-3 questions at a time, not a wall of questions. First confirm whether the user wants a brand-new skill or an improvement to an existing skill. Cover these areas across the conversation:
...
Before running anything, verify the script path exists in the local skills repo. Do not assume `_create-skill`; check the actual directory first if there is any doubt.

Run the init script to scaffold the skill:

```bash
python3 ~/.agents/skills/create-skill/scripts/init_skill.py <skill-name> --path ~/.agents/skills
```
...
Implement scripts, references, and assets identified in Step 2. Test scripts by running them. After scaffolding, explicitly list the generated placeholder files and ask whether to remove any unneeded examples before packaging.
...
Validate and package the skill. Requires `pyyaml` — use a venv if needed. Verify the packaging script path exists before running it:
