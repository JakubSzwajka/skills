...
name: test
description: >
  QA-agent helper for running tests and producing a focused report. Use from QA mode
  or when a workflow delegates validation to the QA agent. Auto-detects test framework
  (pytest, jest, go test, cargo test, etc.), filters noise, and returns actionable results.
