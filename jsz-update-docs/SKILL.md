...
description: Scan the codebase for modules affected by recent changes and ensure their README.md files are up to date. Use when the user asks to update readmes, update docs, sync documentation, or after code changes that may have affected module contracts. Triggers on commands like "update readmes", "sync docs", "readme update". Designed to be composed with other skills (e.g. run readme-update then jsz-commit).
---
